<style type="text/css">
  .beyazyazi{
    color:white;
  }
  #beyazyazi{
    color:white;
  }
</style>
  <aside style="background-color:black" class="main-sidebar sidebar-dark-primary elevation-4">
    <!-- Brand Logo -->
    <a href="#" class="brand-link">
      <img src="dist/img/AdminLTELogo.png" alt="AdminLTE Logo" class="brand-image img-circle elevation-3" style="opacity: .8">
      <span class="brand-text font-weight-light">Caratcons.az</span>
    </a>

    <!-- Sidebar -->
    <div class="sidebar">
      <!-- Sidebar user panel (optional) -->
      <div class="user-panel mt-3 pb-3 mb-3 d-flex">
        <div class="image">
          <img src="dist/img/user2-160x160.jpg" class="img-circle elevation-2" alt="User Image">
        </div>
        <div class="info">
          <a href="#" class="d-block">Caratcons.az</a>
        </div>
      </div>


      <!-- Sidebar Menu -->
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          <li class="nav-item">
            <a href="index.php" class="nav-link active">
              <i class="fas fa-indent"></i>
              <p class="beyazyazi">
                Əsas səhifə
              </p>
            </a>
          </li>
          
         <li class="nav-item">
            <a href="slider.php" class="nav-link ">
              <i id="beyazyazi" class="fas fa-photo-video"></i>
              <p class="beyazyazi">
                Slider
              </p>
            </a>
          </li>
                <li class="nav-item">
            <a href="hakkimizda.php" class="nav-link ">
    <i id="beyazyazi" class="fas fa-address-card"></i>
              <p class="beyazyazi">
                Haqqımızda
              </p>
            </a>
          </li>
          <li class="nav-item menu-open">
            <a href="#" class="nav-link active ">
         <i id="beyazyazi" class="fas fa-users-cog"></i>
              <p class="beyazyazi">
                Tənzimləmələr
                <i class="right fas fa-angle-left"></i>
              </p>
            </a>
            <ul class="nav nav-treeview">
              <li class="nav-item">
                <a href="ayarlar.php?sayfa=ayarlar" class="nav-link ">
                <i id="beyazyazi" class="fas fa-cogs"></i>
                  <p class="beyazyazi">Umumi tənzimləmələr</p>
                </a>
              </li>

          
              <li class="nav-item">
                <a href="ayarlar.php?sayfa=sosyalmedya" class="nav-link">
                 <i id="beyazyazi" class="fab fa-facebook"></i>
                  <p class="beyazyazi">Sosial media hesabları</p>
                </a>
              </li>
            </ul>
          </li>
          <li class="nav-item">
            <a href="blog.php" class="nav-link">
        <i id="beyazyazi" class="fa fa-comment"></i>             
 <p class="beyazyazi">
         Xəbərlər
              </p>
            </a>
          </li>
         
         
         
               <li class="nav-item">
            <a href="xidmet.php" class="nav-link">
          <i id="beyazyazi" class="fas fa-images"></i>
              <p class="beyazyazi">
           Xidmətlərimiz
              </p>
            </a>
          </li>
         
         
 
            <li class="nav-item">
            <a href="elaqe.php" class="nav-link">
          <i id="beyazyazi" class="fas fa-images"></i>
              <p class="beyazyazi">
   Gələn mesajlar
              </p>
            </a>
          </li>
         
     
         


              <li class="nav-item">
            <a href="cikis.php" class="nav-link">
<i id="beyazyazi" class="fas fa-sign-out-alt"></i>
              <p class="beyazyazi">
          Çıxış
              </p>
            </a>
          </li>
        </ul>
      </nav>
      <!-- /.sidebar-menu -->
    </div>
    <!-- /.sidebar -->
  </aside>